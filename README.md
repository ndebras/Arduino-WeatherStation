...
- **Reusable Network Module**: Wi-Fi connection and NTP synchronization helpers extracted into shared source files for reuse across sketches
...
- The heat generated by the Arduino Nano ESP32 controller skews the temperature readings measured by the ASAIR DHT20 sensor. This sensor may not be the most suitable choice and/or the enclosure design may not be appropriate.
...
Several design issues have been identified.

The first one concerns the DHT20 sensor. It only measures temperature and humidity, and does so with limited accuracy.

The second issue comes from the enclosure design itself. It is a simple cylinder, with no separation between the Arduino Nano ESP32 and the sensor. The ESP32 generates heat, which affects the DHT20 measurements.

I will rethink the entire project and implement the following changes:

- Redesign the enclosure to improve ventilation and thermal isolation between the controller and the sensor, while ensuring it remains printable with a standard 3D printer using PLA.
- Replace the DHT20 sensor with a CJMCU-680 module based on the Bosch BME680. This will allow measurement of additional environmental parameters. I still need to investigate how to integrate it into the enclosure without nearby materials (PLA, glue, etc.) affecting its readings.
- A lot of work remains to be done and optimized — exciting!
...
| `WeatherWifi.h/.cpp` | Shared Wi-Fi connection, IP formatting, and NTP synchronization helpers |
...
│           ├── WeatherWifi.cpp         # Shared Wi-Fi and NTP helper implementation
│           ├── WeatherWifi.h           # Shared Wi-Fi and NTP helper declarations
